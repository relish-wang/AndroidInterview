...
SparseArray和HashMap对比
- 1 避免自动装箱, 防止无意义的大量内存分配
- 2 采用二分查找(发生冲突时, 会从冲突位置向上向下继续寻找, 直到找到相同key)
- 3 HashMap查询速度更快, 但内存占用更大
- 4 移除数据的时候会compaction(压实)和resize(调整数组大小)

#### 什么时候用SparseArray?
元素数量在1000以内, 没有大量增删操作。存在大量迭代。
选用的核心就是是否**需要牺牲CPU计算量来节省内存空间**。

## ⑾ ArrayMap

SparseArray和HashMap的内存折中方案。
